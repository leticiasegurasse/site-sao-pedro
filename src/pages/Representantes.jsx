...
const Representantes = () => {
  ...
  // Função para normalizar contatos (array ou objeto único)
  const getStateContacts = (stateCode) => {
    const stateData = contatos[stateCode];
    if (!stateData) {
      return [{
        nome: 'Em breve',
        cidade: 'Em breve',
    ...
    return Array.isArray(stateData) ? stateData : [stateData];
  };

  // Scroll automático quando o estado é selecionado
  useEffect(() => {
    if (selectedState && stateRefs.current[selectedState]) {
      setTimeout(() => {
        stateRefs.current[selectedState].scrollIntoView({
          behavior: 'smooth',
          block: 'center',
          inline: 'nearest'
        });
      ...
      <div className="p-6">
        ...
        {Object.entries(regioes).map(([regiao, estados]) => (
          <div key={regiao} className="mb-6">
            <h4 className="text-lg font-bold text-red-600 mb-3 border-b-2 border-red-600 pb-1">
              {regiao}
            </h4>
            <div className="space-y-2">
              {estados.map(sigla => {
                const contactsList = getStateContacts(sigla);
                const isSelected = selectedState === sigla;
                const hasMultipleContacts = contactsList.length > 1;

                // Contar total de telefones de todos os contatos
                const totalPhones = contactsList.reduce((total, contato) => {
                  let phoneCount = 0;
                  if (contato.telefone) phoneCount++;
                  if (contato.telefone2) phoneCount++;
                  if (contato.telefone3) phoneCount++;
                  if (contato.telefone4) phoneCount++;
                  if (contato.telefoneFixo) phoneCount++;
                  return total + phoneCount;
                }, 0);

                return (
                  <button
                    key={sigla}
                    ref={el => stateRefs.current[sigla] = el}
                    onClick={() => handleStateClick(sigla)}
                    className={`w-full border rounded-lg p-3 text-left transition-all duration-300 ${isSelected
                        ...
                        <div className="flex gap-2">
                          {hasMultipleContacts && (
                            <span className="bg-blue-100 text-blue-600 text-xs px-2 py-1 rounded-full font-medium">
                              {contactsList.length} representantes
                            </span>
                          )}
                          {totalPhones > 1 && (
                            <span className="bg-green-100 text-green-600 text-xs px-2 py-1 rounded-full font-medium">
                              {totalPhones} telefones
                            </span>
                          )}
                        </div>
                      </div>
                      <span className={`transform transition-transform duration-300 ${isSelected ? 'rotate-180 text-red-600' : 'text-gray-500'
                        }`}>
                        ▼
                      </span>
                    </div>
                    ...
                    <AnimatePresence initial={false}>
                      {isSelected && (
                        <motion.div
                          ...
                          className="overflow-hidden mt-4 pt-4 border-t border-gray-200 space-y-4"
                        >
                          {contactsList.map((contato, index) => (
                            <ContactDisplay
                              key={index}
                              contato={contato}
                              index={index}
          ...
          <div className="w-full bg-white py-16">
            <Container>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-[600px]">
                ...
                {/* Coluna da Lista */}
                <div className="flex flex-col">
                  <div className="bg-gray-50 rounded-lg shadow-inner max-h-[600px] overflow-hidden">
                    {renderStateList()}
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {estados.map(sigla => {
                    ...
                    return (
                      <button
                        key={sigla}
                        onClick={() => setSelectedState(isSelected ? null : sigla)}
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`text-red-600 transform transition-transform duration-300 ${isSelected ? 'rotate-180' : ''
                            }`}>
                            ▼
                          </span>
                        </div>
...
// Componente do mapa (mesmo código anterior)
const BrazilMapInteractiveDesktop = ({ selectedState, onStateClick }) => {
  const estadosNomes = {
    'AC': 'Acre', 'AL': 'Alagoas', 'AP': 'Amapá', 'AM': 'Amazonas',
    'BA': 'Bahia', 'CE': 'Ceará', 'DF': 'Distrito Federal', 'ES': 'Espírito Santo',
    'GO': 'Goiás', 'MA': 'Maranhão', 'MT': 'Mato Grosso', 'MS': 'Mato Grosso do Sul',
  ...
  const handleStateHover = (event) => {
    ...
    if (svg && event.target.tagName === 'path') {
      svg.appendChild(event.target);
    }
  };

  const handleMapClick = (event) => {
    const estadoCode = event.target.getAttribute('data-estado');
    if (estadoCode && onStateClick) {
      onStateClick(estadoCode);
    }
  };

  const getStateClasses = (stateCode) => {
    return `stroke-white stroke-1 transition-all duration-300 cursor-pointer ${selectedState === stateCode
      ? 'fill-red-600 scale-101'
      : 'fill-gray-300 hover:fill-red-600 hover:scale-101'
      }`;
  };
