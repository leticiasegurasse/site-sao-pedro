  ...
  return (
    <section className="max-w-[1200px] min-h-[500px] mx-auto px-4 py-16">
      ...
      <div className="flex flex-col md:flex-row md:flex-wrap gap-6 justify-between items-center">
        {[1, 2, 3, 4].map((item) => (
          <div key={item} className="group bg-gray-100 overflow-hidden flex flex-col transition-all duration-500 ease-in-out w-full md:w-[calc(25%-18px)] flex-1 min-w-[260px] max-w-[350px] mx-auto relative">
            <Link to="/detalhes-produto">
              <div className="relative">
                <img src="/images/carvoeira.png" alt="Base Carvoeira" className="w-full h-55 object-cover transition-all duration-500 group-hover:h-40" />
                <div className="absolute left-0 bottom-0 w-0 h-1 bg-red-600 group-hover:w-full transition-all duration-300"></div>
              </div>
              <div className="pt-4 px-4 flex-1 flex flex-col justify-end relative z-10">
                <span className="text-2xl font-extrabold mb-2">Base Carvoeira</span>
              </div>
