const categorias = [
  {
    nome: "Linha Baú",
    slug: "linha-bau",
    imagem: "/images/semirreboque-furgao1.jpg",
    descricao: "Soluções fechadas para proteção de cargas com altura de 3011mm"
  },
  {
    nome: "Linha Sider",
    slug: "linha-sider",
    imagem: "/images/semirreboque-sider1.jpg",
    descricao: "Facilidade no carregamento lateral com altura de 3056mm"
  },
  {
    nome: "Linha Carga Seca",
    slug: "linha-carga-seca",
    imagem: "/images/semirreboques.jpg",
    descricao: "Versatilidade para diversos tipos de carga com tampas ACM"
  },
  {
    nome: "Linha Graneleira",
    slug: "linha-graneleira",
    imagem: "/images/carreta-graneleira1.jpg",
    descricao: "Transporte de granéis com sistema de descarga eficiente"
  },
  {
    nome: "Linha Porta Container (VPC)",
    slug: "linha-porta-container",
    imagem: "/images/vpc-buggy1.jpg",
    descricao: "Especializada em transporte de containers 20 e 40 pés"
  },
  {
    nome: "Linha Base",
    slug: "linha-base",
    imagem: "/images/base-carvoeira1.jpg",
    descricao: "Bases carvoeiras e frigoríficas para cargas especiais"
  }
];

export default categorias;